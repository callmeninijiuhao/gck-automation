...
// By Country (top 10 + DoD) → By Ad Format.
// ─────────────────────────────────────────────
...
/** Ad format pivot: one bold row per format (share of total + PMR vs prev). Ad-size sub-rows are intentionally omitted from the email. */
function pivotHtml(groups: AdFormatGroup[], dod: DimDayOverDay | null): string {
  if (!groups.length) return '<p><i>No data.</i></p>';
  const rows: string[][] = groups.map((g) => [
    `<b>${esc(g.adFormat)}</b>`, `<b>${fmtCurrency(g.spend)}</b>`, `<b>${fmtPct(g.spendShare)}</b>`, `<b>${fmtCurrency(g.pmr)}</b>`, `<b>${fmtPct(g.pmrShare)}</b>`, `<b>${dimCell(dod, g.adFormat)}</b>`, `<b>${fmtEcpm(g.ecpm)}</b>`,
  ]);
  return htmlTable(
    [{ label: 'Ad Format' }, { label: 'DSP Spend', align: 'right' }, { label: 'DSP %', align: 'right' }, { label: 'PMR', align: 'right' }, { label: 'PMR %', align: 'right' }, { label: 'vs prev', align: 'right' }, { label: 'eCPM', align: 'right' }],
    rows,
  );
}
...
export function buildEmailHtml(
  ...
  return `
    ...
    <h3>9. By Ad Format</h3>
    ${pivotHtml(s.adFormatPivot, dod.adFormat)}
    <p style="font-size:10pt;color:#777;margin:4px 0 0">Format rows = share of total in-app (DSP % / PMR %) with PMR vs prev.</p>
